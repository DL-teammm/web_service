...
3. После совершённого выбора на основной странице веб-сервиса в процессе обработки видео на каждом 20-ом кадре будут печататься логи, которые содержат информацию о том, сколько дорожных знаков было обнаружено на данном кадре, какое в среднем FPS было у детекционной модели на последних кадрах.

4. После завершения обработки тестового клипа будут сохранены следующие результаты:  

    - видео с отрисовкой детекций в папке: ```./results/videos/```

    - текстовый файл с логами запуска обработки в папке: ```./results/logs/```

5. Видео с отрисовкой результатов также будет отображаться на основной странице веб-сервиса.
